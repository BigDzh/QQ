    ...
    try {
      ...
      if (maxRecords !== undefined) {
        ...
        if (allRecords.length > maxRecords) {
          const sortedRecords = allRecords.sort((a: unknown, b: unknown) => {
            const recordA = a as Record<string, unknown>;
            const recordB = b as Record<string, unknown>;
            const timeA = (recordA.timestamp || recordA.createdAt || 0) as number;
            const timeB = (recordB.timestamp || recordB.createdAt || 0) as number;
            return timeA - timeB;
          });
