...
import { safeSetObject, safeGetItem } from './storageManager';
...
export function shouldRemindBackup(): boolean {
  ...
  const lastReminder = safeGetItem<string>(LAST_BACKUP_REMINDER_KEY);
...
export function recordBackupReminder(): void {
  safeSetObject(LAST_BACKUP_REMINDER_KEY, new Date().toISOString());
}
