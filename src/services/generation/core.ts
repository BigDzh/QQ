...
export function initGenerationSystem(partialConfig?: Partial<GenerationSystemConfig>): void {
  ...
  initialized = true;
  logEvent('GENERATOR_REGISTERED', 'INFO', '生成系统初始化完成', { config: systemConfig });
}
...
class GenerationRegistryClass {
  register(entry: Omit<RegistryEntry, 'id' | 'firstGeneratedAt' | 'lastGeneratedAt' | 'generationCount' | 'forceRegenerationCount'>): RegistryEntry {
    ...
    this.persist();
    logEvent('REGISTRY_UPDATED', 'INFO', `注册表更新: ${entry.typeKey}`, { entryId: newEntry.id, typeKey: entry.typeKey });
    return newEntry;
  }
  ...
  setCooldown(typeKey: GenerationTypeKey, projectId: string, durationMs?: number): void {
    ...
    this.persist();
    logEvent('COOLDOWN_SET', 'INFO', `冷却期设置: ${typeKey}`, { typeKey, durationMs: cooldownDuration });
  }
  ...
  validate(
    ...
    if (!isForced && GenerationRegistry.isInCooldown(typeKey, projectId)) {
      ...
      issues.push({
        ...
        severity: 'warning',
        message: `类型 "${typeKey}" 处于冷却期内，剩余 ${Math.ceil(remaining / 1000)} 秒`,
        bypassable: true,
...
class InterfaceGeneratorClass {
  async generate<T>(params: GenerateParams<T>): Promise<GenerateResult<T>> {
    ...
    if (!generatorConfig) {
      const error = { code: 'GENERATOR_NOT_FOUND', message: `未找到类型 ${typeKey} 的生成器` };
    ...
    logEvent('GENERATION_STARTED', 'INFO', `开始执行生成: ${typeKey}`, { typeKey, projectId });
