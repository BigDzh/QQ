...
const STORAGE_INFO_CACHE_TTL = 5000;
let storageInfoCache: { data: StorageStatus; timestamp: number } | null = null;
...
export function getStorageInfo(): StorageStatus {
  const now = Date.now();
  if (storageInfoCache && now - storageInfoCache.timestamp < STORAGE_INFO_CACHE_TTL) {
    return storageInfoCache.data;
  }
  ...
  storageInfoCache = { data: result, timestamp: now };
  return result;
}
