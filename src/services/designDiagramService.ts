import type { Project, Module, Component, ProjectStage } from '../types';
...
export function generateDesignDiagrams(project: Project): DiagramCard[] {
  ...
  project.modules.forEach((module, moduleIndex) => {
    const moduleContent = generateSingleModuleAssemblyForCAD(module, project, moduleIndex + 1);
    cards.push({
      id: `module-assembly-${module.id}-${Date.now()}`,
    ...
    module.components.forEach((component, compIndex) => {
      const compContent = generateSingleComponentAssemblyForCAD(component, module, compIndex + 1);
      cards.push({
        id: `component-assembly-${component.id}-${Date.now()}`,
      ...
      cards.push({
        id: `component-table-${component.id}-${Date.now()}`,
...
function generateSingleModuleAssemblyForCAD(module: Module, project: Project, moduleNo: number): string {
  const lines: string[] = [];
  const width = 120;
  ...
  lines.push(padCenter(`Module No: ${moduleNo} | Stage: ${module.stage} | Version: ${module.version}`, width));
  lines.push(repeatChar('-', width));
  lines.push('');
...
function generateSingleComponentAssemblyForCAD(component: Component, module: Module, compNo: number): string {
  const lines: string[] = [];
  const width = 120;

  lines.push(repeatChar('-', width));
  lines.push(padCenter(`COMPONENT ASSEMBLY - ${component.componentName}`, width));
  lines.push(padCenter(`Module: ${module.moduleName} | Component No: ${compNo}`, width));
  lines.push(repeatChar('-', width));
  lines.push('');
