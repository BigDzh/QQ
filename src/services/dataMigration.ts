...
export async function createPreUpdateSnapshot(): Promise<DataSnapshot> {
  try {
    ...
    try {
      ...
      documentCount = projects.reduce((sum: number, p: ParsedProject) => sum + (p.documents?.length || 0), 0);
    } catch {
      // Ignore parse errors
    }
    ...
    throw new Error(`创建数据快照失败: ${error instanceof Error ? error.message : '未知错误'}`);
  }
}
...
export async function validateImportData(jsonStr: string): Promise<{ valid: boolean; error?: string; data?: unknown }> {
  try {
...
export function getDataStats(): { projectCount: number; userCount: number; documentCount: number; backupCount: number } {
  ...
  try {
    ...
    if (projects) {
      ...
      documentCount = parsed.reduce((sum: number, p: Record<string, unknown>) => sum + (((p as any).documents?.length || 0)), 0);
    }
  } catch {
    // Ignore parse errors
  }
