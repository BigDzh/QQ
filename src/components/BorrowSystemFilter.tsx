...
import { X, Search, Filter, Layers, Box, Cpu, FolderOpen, ChevronRight, Check } from 'lucide-react';
...
export default function FilterArea({ projects, onFilterChange }: FilterAreaProps) {
  ...
  }, [projects, selectedProject, selectedModule]);
  ...
  const getProjectLabel = () => {
    if (!selectedProject) return '请选择项目';
    const project = projects.find(p => p.id === selectedProject);
    return project ? project.name : '请选择项目';
  };
  ...
  const handleProjectSelect = (projectId: string) => {
    ...
    onFilterChange({ projectId, systemId: null, moduleId: null, componentId: null });
  };
  ...
  const handleSystemSelect = (systemId: string) => {
    ...
    onFilterChange({ projectId: selectedProject, systemId, moduleId: null, componentId: null });
  };
  ...
  const handleModuleSelect = (moduleId: string) => {
    ...
    onFilterChange({ projectId: selectedProject, systemId: selectedSystem, moduleId, componentId: null });
  };
  ...
  const handleComponentSelect = (componentId: string) => {
    ...
    onFilterChange({ projectId: selectedProject, systemId: selectedSystem, moduleId: selectedModule, componentId });
  };
  ...
  const handleClearProject = (e: React.MouseEvent) => {
    ...
    setSelectedModule(null);
    setSelectedComponent(null);
    onFilterChange({ projectId: null, systemId: null, moduleId: null, componentId: null });
  };
  ...
  const handleClearSystem = (e: React.MouseEvent) => {
    ...
    setSelectedModule(null);
    setSelectedComponent(null);
    onFilterChange({ projectId: selectedProject, systemId: null, moduleId: null, componentId: null });
  };
  ...
  const handleClearModule = (e: React.MouseEvent) => {
    ...
    setSelectedModule(null);
    setSelectedComponent(null);
    onFilterChange({ projectId: selectedProject, systemId: selectedSystem, moduleId: null, componentId: null });
  };

  const handleClearComponent = (e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedComponent(null);
    onFilterChange({ projectId: selectedProject, systemId: selectedSystem, moduleId: selectedModule, componentId: null });
  };
    ...
    <div className={`${t.card} rounded-lg shadow-sm p-4 border ${t.border} mb-6`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          ...
          >
            <span className={`text-sm truncate ${selectedProject ? t.text : t.textMuted}`}>
              {getProjectLabel()}
            </span>
            {selectedProject ? (
              <button
        ...
        <div>
          ...
          <button
            ...
            className={`w-full min-h-[42px] px-3 py-2 border rounded-lg text-left flex items-center justify-between ${t.input} ${!selectedProject ? 'opacity-50 cursor-not-allowed' : 'hover:border-blue-500 transition-colors cursor-pointer'}`}
          >
            <span className={`text-sm truncate ${selectedSystem ? t.text : t.textMuted}`}>
              {selectedProject ? getSystemLabel() : '请先选择项目'}
            </span>
            {selectedSystem ? (
              <button
      ...
      <Modal
        ...
        title="选择项目"
        icon={<FolderOpen size={20} className="text-orange-500" />}
      >
        {renderSearchInput(projectSearch, setProjectSearch, '搜索项目名称...')}
        {renderList(
          filteredProjects,
          selectedProject,
          handleProjectSelect,
          (project) => (
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {project.modules.length} 个模块 | {project.systems.length} 个系统
              </div>
            </div>
          )
        )}
      ...
      {activeFiltersCount > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {selectedProject && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-orange-500/20 text-orange-500">
                项目: {projects.find(p => p.id === selectedProject)?.name}
                <button onClick={handleClearProject} className="hover:text-orange-300">
                  <X size={12} />
                </button>
              </span>
            )}
