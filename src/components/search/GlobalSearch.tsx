...
import { Search, X, FileText, FolderKanban, Package, CheckSquare, Boxes, ArrowRight, Filter, Clock, Trash2, Monitor } from 'lucide-react';
...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    if (location.state?.openSearch) {
      ...
      import('react').then(({ startTransition }) => {
        startTransition(() => navigate(location.pathname, { replace: true }));
      });
    }
  }, [location, navigate]);
  ...
  const handleSelect = useCallback((result: SearchResult) => {
    ...
    setQuery('');
    import('react').then(({ startTransition }) => {
      startTransition(() => navigate(result.path));
    });
  }, [query, navigate]);
  ...
  const getTypeIcon = useCallback((type: string) => {
    switch (type) {
      ...
      case 'system':
        return <Monitor size={16} className="text-red-500" />;
      case 'module':
