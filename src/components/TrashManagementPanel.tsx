...
export const TrashManagementPanel: React.FC<TrashManagementPanelProps> = ({
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; type: 'empty' | 'restore' | 'delete' | null; item?: TrashTask }>({
    show: false,
    type: null,
    item: undefined,
  });
  ...
  return (
          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'trash' && trashItems.length > 0 && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    setDeleteConfirm({ show: true, type: 'empty' });
                  }}
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          {recoverable ? (
                            <>
                              ...
                              <button
                                onClick={() => {
                                  setDeleteConfirm({ show: true, type: 'restore', item });
                                }}
                          ...
                          <button
                            onClick={() => {
                              setDeleteConfirm({ show: true, type: 'delete', item });
                            }}
    ...
    <ConfirmModal
      ...
      title={deleteConfirm.type === 'empty' ? '确认清空回收站' : deleteConfirm.type === 'restore' ? '确认恢复任务' : '确认永久删除'}
      message={
        <div>
          {deleteConfirm.type === 'empty' && (
            <p>确定要清空回收站吗？这将永久删除 <strong>{trashItems.length} 个任务</strong>。此操作不可撤销。</p>
          )}
          {deleteConfirm.type === 'restore' && (
            <p>确定要恢复任务 <strong>"{deleteConfirm.item?.originalTask.title}"</strong> 吗？</p>
          )}
          {deleteConfirm.type === 'delete' && (
            <p>确定要永久删除任务 <strong>"{deleteConfirm.item?.originalTask.title}"</strong> 吗？此操作不可恢复！</p>
          )}
          <p className="text-red-500 mt-2">此操作不可撤销，请谨慎操作。</p>
        </div>
      }
      onConfirm={() => {
        if (deleteConfirm.type === 'empty') {
          onEmptyTrash();
        } else if (deleteConfirm.type === 'restore' && deleteConfirm.item) {
          const success = onRestore(deleteConfirm.item.id);
          if (!success) {
            alert('恢复失败，任务可能已过期');
          }
        } else if (deleteConfirm.type === 'delete' && deleteConfirm.item) {
          onPermanentDelete(deleteConfirm.item.id);
        }
        setDeleteConfirm({ show: false, type: null, item: undefined });
      }}
      onCancel={() => setDeleteConfirm({ show: false, type: null, item: undefined })}
      confirmText="确认"
      cancelText="取消"
      type="danger"
    />
