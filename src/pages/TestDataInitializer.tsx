  ...
  const getCurrentDataStats = useCallback(() => {
    ...
    try {
      ...
      if (projectsData) {
        ...
        modules = parsed.reduce((sum: number, p: Record<string, unknown>) => sum + ((p.modules && Array.isArray(p.modules)) ? p.modules.length : 0), 0);
        components = parsed.reduce((sum: number, p: Record<string, unknown>) =>
          sum + ((p.modules && Array.isArray(p.modules)) ? p.modules.reduce((s: number, m: Record<string, unknown>) => s + ((m.components && Array.isArray(m.components)) ? m.components.length : 0), 0) : 0), 0);
      }
    } catch {
      // Ignore parse errors
    }
