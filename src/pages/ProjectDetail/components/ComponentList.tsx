...
import { Package, Copy, Trash2, Edit2, Search, X, Plus, CheckSquare, Square, ArrowUp, ArrowDown, LayoutGrid } from 'lucide-react';
...
interface ComponentRowProps {
  ...
  onDelete: (moduleId: string, componentId: string) => void;
  onStatusClick: (comp: ComponentWithModule) => void;
  canEdit: boolean;
  statusColors: Record<string, string>;
}

const ComponentRow = memo(function ComponentRow({
  comp, isSelected, t, onSelect, onNavigate, onEdit, onCopy, onDelete, onStatusClick, canEdit, statusColors,
}: ComponentRowProps) {
  return (
    <div
      className={`grid items-center gap-2 px-4 border-t ${t.border} ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''} ${t.hoverBg}`}
      style={{ gridTemplateColumns: '40px 1fr 1.5fr 1fr 80px 80px 100px 120px', height: `${ROW_HEIGHT}px` }}
    >
      <div onClick={(e) => e.stopPropagation()}>
        <button
          onClick={() => onSelect(comp.id)}
          className={`p-1 rounded transition-colors ${
            isSelected ? 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'
          }`}
      ...
      <div onClick={() => onNavigate(`/components/${comp.id}`)} className={`${t.text} cursor-pointer truncate`}>{comp.componentNumber}</div>
      <div><Link to={`/components/${comp.id}`} className={`${t.text} hover:underline font-medium truncate block`}>{comp.componentName}</Link></div>
      <div className={`${t.textSecondary} truncate`}>{comp.moduleName || '-'}</div>
      <div className={`${t.textSecondary} truncate`}>{comp.stage || '-'}</div>
      <div className={`${t.textSecondary} truncate`}>{comp.version || '-'}</div>
      <div onClick={(e) => e.stopPropagation()}>
        <button
          onClick={() => onStatusClick(comp)}
          className={`px-2 py-1 rounded text-xs transition-colors cursor-pointer ${statusColors[comp.status] || statusColors['故障']} hover:opacity-80`}
        >{comp.status}</button>
      </div>
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        <button onClick={(e) => { e.stopPropagation(); onEdit(comp); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="编辑"><Edit2 size={16} /></button>
        <button onClick={(e) => { e.stopPropagation(); onCopy(comp); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="复制组件"><Copy size={16} /></button>
        {canEdit && (
          <button onClick={() => { onDelete(comp.moduleId, comp.id); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="删除"><Trash2 size={16} /></button>
        )}
      </div>
    </div>
  );
...
export function ComponentList({
  ...
  const [deleteComponentId, setDeleteComponentId] = useState<{ moduleId: string; componentId: string } | null>(null);
  ...
  const filteredComponents = useMemo(() => {
    return allComponents
      ...
      .sort((a: ComponentWithModule, b: ComponentWithModule) => {
        const aVal = String(a[sortField as keyof ComponentWithModule] || '');
        const bVal = String(b[sortField as keyof ComponentWithModule] || '');
  ...
  const useVirtualScroll = filteredComponents.length > VIRTUAL_SCROLL_THRESHOLD;
  ...
  const handleRowNavigate = useCallback((path: string) => {
    import('react').then(({ startTransition }) => {
      startTransition(() => navigate(path));
    });
  }, [navigate]);
  const handleRowEdit = useCallback((comp: Component) => { onEditComponent(comp); }, [onEditComponent]);
  const handleRowCopy = useCallback((comp: Component) => { onCopyComponent(comp); }, [onCopyComponent]);
  const handleRowDelete = useCallback((moduleId: string, componentId: string) => { onDeleteComponent(moduleId, componentId); }, [onDeleteComponent]);
  const handleRowStatusClick = useCallback((comp: ComponentWithModule) => {
    if (onStatusChangeWithReason && canEdit) {
      setEnhancedStatusModal({ isOpen: true, component: comp as Component, currentStatus: comp.status as ComponentStatus });
    } else if (onStatusChange) {
      onStatusChange(comp);
    }
  }, [onStatusChangeWithReason, onStatusChange, canEdit]);

  const displayStatusColors = useMemo(() => ({
    '未投产': t.statusColors['未投产'] || 'bg-gray-400',
    '投产中': t.statusColors['投产中'] || 'bg-blue-500',
    '正常': t.statusColors['正常'] || 'bg-green-500',
    '维修中': t.statusColors['维修中'] || 'bg-orange-500',
    '三防中': t.statusColors['三防中'] || 'bg-purple-500',
    '测试中': t.statusColors['测试中'] || 'bg-yellow-500',
    '仿真中': t.statusColors['仿真中'] || 'bg-cyan-500',
    '借用中': t.statusColors['借用中'] || 'bg-pink-500',
    '故障': t.statusColors['故障'] || 'bg-red-500',
  }), [t.statusColors]);
  ...
  return (
    <div className="space-y-4">
          ...
          useVirtualScroll ? (
            <div className="border rounded-lg overflow-hidden" style={{ borderColor: 'var(--tw-border-color, #e5e7eb)' }}>
              <div className={`grid items-center gap-2 px-4 py-3 text-sm font-medium ${t.tableHeader} ${t.textSecondary}`}
                style={{ gridTemplateColumns: '40px 1fr 1.5fr 1fr 80px 80px 100px 120px' }}
              >
                <div>
                  <button onClick={handleToggleAll} className={`p-1 rounded transition-colors ${isAllSelected || isIndeterminate ? 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`} aria-label={isAllSelected ? '取消全选' : '全选'}>
                    {isAllSelected || isIndeterminate ? <CheckSquare size={18} className={isIndeterminate ? 'opacity-50' : ''} /> : <Square size={18} className="opacity-50" />}
                  </button>
                </div>
                <div><button onClick={() => handleSort('componentNumber')} className="flex items-center gap-1 hover:text-blue-500 cursor-pointer">组件编号<SortIndicator field='componentNumber' /></button></div>
                <div><button onClick={() => handleSort('componentName')} className="flex items-center gap-1 hover:text-blue-500 cursor-pointer">组件名称<SortIndicator field='componentName' /></button></div>
                <div><button onClick={() => handleSort('moduleName')} className="flex items-center gap-1 hover:text-blue-500 cursor-pointer">所属模块<SortIndicator field='moduleName' /></button></div>
                <div>阶段</div>
                <div>版本</div>
                <div>状态</div>
                <div>操作</div>
              ...
              <AutoVirtualList
                ...
                overscan={5}
                getItemKey={(comp) => comp.id}
                renderItem={(comp) => (
                  <ComponentRow
                    comp={comp}
                    isSelected={selectedComponents.has(comp.id)}
                    t={t}
            ...
            <table className="w-full">
              <thead className={t.tableHeader}>
                <tr>
                  ...
                  <th className={`px-4 py-3 text-left text-sm font-medium ${t.textSecondary}`}><button onClick={() => handleSort('componentNumber')} className={`flex items-center gap-1 hover:text-blue-500 cursor-pointer transition-colors ${t.textSecondary}`}>组件编号<SortIndicator field='componentNumber' /></button></th>
                  <th className={`px-4 py-3 text-left text-sm font-medium ${t.textSecondary}`}><button onClick={() => handleSort('componentName')} className={`flex items-center gap-1 hover:text-blue-500 cursor-pointer transition-colors ${t.textSecondary}`}>组件名称<SortIndicator field='componentName' /></button></th>
                  <th className={`px-4 py-3 text-left text-sm font-medium ${t.textSecondary}`}><button onClick={() => handleSort('moduleName')} className={`flex items-center gap-1 hover:text-blue-500 cursor-pointer transition-colors ${t.textSecondary}`}>所属模块<SortIndicator field='moduleName' /></button></th>
              ...
              <tbody>
                {paginatedComponents.map((comp) => {
                  ...
                  return (
                    <tr key={comp.id} className={`border-t ${t.border} ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''} ${t.hoverBg}`}>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => handleSelectComponent(comp.id)} className={`p-1 rounded transition-colors ${isSelected ? 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`} aria-label={isSelected ? '取消选择' : '选择'}>
                          {isSelected ? <CheckSquare size={18} /> : <Square size={18} className="opacity-50" />}
                        </button>
                      </td>
                      <td onClick={() => navigate(`/components/${comp.id}`)} className={`px-4 py-3 ${t.text} cursor-pointer`}>{comp.componentNumber}</td>
                      <td className={`px-4 py-3`}><Link to={`/components/${comp.id}`} className={`${t.text} hover:underline font-medium`}>{comp.componentName}</Link></td>
                      ...
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => { if (onStatusChangeWithReason && canEdit) { setEnhancedStatusModal({ isOpen: true, component: comp as Component, currentStatus: comp.status as ComponentStatus }); } else if (onStatusChange) { onStatusChange(comp); } }} className={`px-2 py-1 rounded text-xs transition-colors cursor-pointer ${t.statusColors[comp.status as keyof typeof t.statusColors] || t.statusColors['故障']} hover:opacity-80`}>{comp.status}</button>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 sm:gap-2" onClick={(e) => e.stopPropagation()}>
                          <button onClick={(e) => { e.stopPropagation(); onEditComponent(comp); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="编辑"><Edit2 size={16} /></button>
                          <button onClick={(e) => { e.stopPropagation(); onCopyComponent(comp); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="复制组件"><Copy size={16} /></button>
                          {canEdit && (<button onClick={(e) => { e.stopPropagation(); setDeleteComponentId({ moduleId: comp.moduleId, componentId: comp.id }); }} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="删除"><Trash2 size={16} /></button>)}
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {useVirtualScroll ? (
        <div className={`flex items-center justify-between px-4 py-2 ${t.card} rounded-lg border ${t.border}`}>
          <div className={`text-sm ${t.textSecondary}`}>共 {filteredComponents.length} 条组件（虚拟滚动模式）</div>
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (deleteComponentId) {
            onDeleteComponent(deleteComponentId.moduleId, deleteComponentId.componentId);
          }
