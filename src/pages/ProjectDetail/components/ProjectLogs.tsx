...
import type { LogLayer } from '../../../services/logger/types';
...
export function ProjectLogs({ logs: propLogs }: ProjectLogsProps) {
    ...
    const loadAllLogs = () => {
      ...
      const combined: UnifiedLogEntry[] = [
        ...projectLogs.map(log => ({
          ...
          layer: 'PROJECT' as LogLayer | 'PROJECT',
          level: log.level,
        })),
        ...systemLogs.map(log => ({
          id: log.id,
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (activeTab !== 'all') {
      const layerMap: Record<string, LogLayer | 'PROJECT'> = {
        'project': 'PROJECT',
        'system': 'SYSTEM',
        'module': 'MODULE',
        'component': 'COMPONENT',
  ...
  const getLayerIcon = (layer: LogLayer | 'PROJECT') => {
    switch (layer) {
      case 'PROJECT':
        return '📋';
      case 'SYSTEM':
