...
import { Plus, Package, Copy, Trash2, Edit2, ChevronRight, Search, X, CheckSquare, Square, LayoutGrid } from 'lucide-react';
...
const MODULE_ROW_HEIGHT = 52;

interface ModuleRowProps {
  m: Module; isSelected: boolean;
  t: ReturnType<typeof useThemeStyles>;
  onSelect: (id: string) => void; onNavigate: (path: string) => void;
  onEdit: (mod: Module) => void; onCopy: (mod: Module) => void;
  onDelete: (id: string) => void; canEdit: boolean;
}

const ModuleRow = memo(function ModuleRow({ m, isSelected, t, onSelect, onNavigate, onEdit, onCopy, onDelete, canEdit }: ModuleRowProps) {
  return (
    <div className={`grid items-center gap-2 px-4 border-t ${t.border} ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''} ${t.hoverBg}`}
      style={{ gridTemplateColumns: '40px 1fr 1.5fr 1fr 80px 80px 100px 120px', height: `${MODULE_ROW_HEIGHT}px` }}>
      <div onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onSelect(m.id)} className={`p-1 rounded transition-colors ${isSelected ? 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
          {isSelected ? <CheckSquare size={18} /> : <Square size={18} className="opacity-50" />}
        </button>
      </div>
      <div onClick={() => onNavigate(`/modules/${m.id}`)} className={`${t.text} cursor-pointer truncate`}>{m.moduleNumber}</div>
      <div><Link to={`/modules/${m.id}`} className={`${t.text} hover:underline font-medium truncate block`}>{m.moduleName}</Link></div>
      <div className={`${t.textSecondary} truncate`}>{m.systemName || '-'}</div>
      <div className={`${t.textSecondary} truncate`}>{m.stage || '-'}</div>
      <div className={`${t.textSecondary} truncate`}>{m.version || '-'}</div>
      <div><span className={`px-2 py-1 rounded text-xs ${t.statusColors[m.status as keyof typeof t.statusColors] || t.statusColors['故障']}`}>{m.status}</span></div>
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onEdit(m)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="编辑"><Edit2 size={16} /></button>
        <button onClick={() => onCopy(m)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="复制"><Copy size={16} /></button>
        {canEdit && (<button onClick={() => onDelete(m.id)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="删除"><Trash2 size={16} /></button>)}
      </div>
    </div>
  );
});
...
export function ModuleList({
  ...
  const useVirtualScroll = filteredModules.length > VIRTUAL_SCROLL_THRESHOLD;
  ...
  const stableEdit = useCallback((m: Module) => onEditModule(m), [onEditModule]);
  const stableCopy = useCallback((m: Module) => onCopyModule(m, `${m.moduleNumber}-copy`), [onCopyModule]);
  const stableDelete = useCallback((id: string) => onDeleteModule(id), [onDeleteModule]);
  ...
  const handleRowClick = (e: React.MouseEvent, moduleId: string) => {
    ...
    import('react').then(({ startTransition }) => {
      startTransition(() => navigate(`/modules/${moduleId}`));
    });
  };
    ...
    <div className="space-y-4">
          ...
          useVirtualScroll ? (
            <div className="border rounded-lg overflow-hidden" style={{ borderColor: 'var(--tw-border-color, #e5e7eb)' }}>
              <div className={`grid items-center gap-2 px-4 py-3 text-sm font-medium ${t.tableHeader} ${t.textSecondary}`}
                style={{ gridTemplateColumns: canEdit ? '40px 1fr 1.5fr 1fr 80px 80px 100px 80px 120px' : '1fr 1.5fr 1fr 80px 80px 100px 80px 120px' }}>
                {canEdit && (<div>选择</div>)}
                <div>模块编号</div><div>模块名称</div><div>种类</div>
                <div>阶段</div><div>版本</div><div>状态</div><div>组件数</div><div>操作</div>
              </div>
              <AutoVirtualList items={filteredModules} height={Math.min(600, window.innerHeight - 350)} estimatedItemHeight={MODULE_ROW_HEIGHT} overscan={5}
                getItemKey={(m) => m.id}
                renderItem={(m) => (
                  <ModuleRow m={m} isSelected={selectedModuleIds.has(m.id)} t={t} onSelect={handleToggleModule}
                    onNavigate={stableNavigate} onEdit={stableEdit} onCopy={stableCopy} onDelete={stableDelete} canEdit={canEdit} />
                )}
              />
            </div>
          ) : (
      ...
      {useVirtualScroll && (
        <div className={`flex items-center justify-between px-4 py-2 ${t.card} rounded-lg border ${t.border} mt-3`}>
          <div className={`text-sm ${t.textSecondary}`}>共 {filteredModules.length} 条模块（虚拟滚动模式）</div>
