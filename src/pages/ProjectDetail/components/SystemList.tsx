...
import { Plus, Monitor, Edit2, Trash2, ChevronRight, Clock, CheckSquare, Square, ArrowUp, ArrowDown, LayoutGrid } from 'lucide-react';
...
import { STAGE_OPTIONS } from '../../../services/stageConfig';
...
const SYSTEM_ROW_HEIGHT = 56;

interface SystemRowProps {
  sys: System; isSelected: boolean; modules: Module[];
  t: ReturnType<typeof useThemeStyles>;
  onSelect: (id: string) => void; onNavigate: (path: string) => void;
  onEdit: (s: System) => void; onDelete: (id: string) => void; canEdit: boolean;
  getSystemStatusInfo: (s: System) => { total: number; statusCounts: Record<string, number>; statusColors: Record<string, string>; statusBg: string };
}

const SystemRow = memo(function SystemRow({ sys, isSelected, modules: _modules, t, onSelect, onNavigate, onEdit, onDelete, canEdit, getSystemStatusInfo }: SystemRowProps) {
  const { total, statusCounts, statusColors, statusBg } = getSystemStatusInfo(sys);
  return (
    <div className={`grid items-center gap-2 px-4 border-t ${t.border} ${isSelected ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''} ${t.hoverBg}`}
      style={{ gridTemplateColumns: '40px 1.5fr 1fr 1fr 80px 100px 120px', height: `${SYSTEM_ROW_HEIGHT}px` }}>
      <div onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onSelect(sys.id)} className={`p-1 rounded transition-colors ${isSelected ? 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
          {isSelected ? <CheckSquare size={18} /> : <Square size={18} className="opacity-50" />}
        </button>
      </div>
      <div onClick={() => onNavigate(`/systems/${sys.id}`)} className={`${t.text} cursor-pointer truncate`}>{sys.systemName}</div>
      <div className={`${t.textSecondary} truncate`}>{sys.systemNumber}</div>
      <div className={`${t.textSecondary} truncate`}>{sys.productionOrderNumber}</div>
      <div className={`${t.textSecondary} truncate`}>{sys.stage}</div>
      <td>
        <div className="relative inline-block">
          <span className={`px-2 py-1 rounded text-xs ${statusBg}`}>{sys.status}</span>
          {total > 0 && (
            <div className="absolute bottom-0 left-0 right-0 h-1 flex rounded-b overflow-hidden">
              {Object.entries(statusCounts).map(([st, count]) => (
                <div key={st} className="h-full" style={{ width: `${(count / total) * 100}%`, backgroundColor: statusColors[st] || '#6b7280' }} />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onEdit(sys)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="编辑"><Edit2 size={16} /></button>
        {canEdit && (<button onClick={() => onDelete(sys.id)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="删除"><Trash2 size={16} /></button>)}
      </div>
    </div>
  );
});
...
export function SystemList({
  ...
  const useVirtualScroll = filteredSystems.length > VIRTUAL_SCROLL_THRESHOLD;
  ...
  const handleSelectSystem = (systemId: string) => {
    ...
    if (newSelected.has(systemId)) { newSelected.delete(systemId); } else { newSelected.add(systemId); }
    setSelectedSystems(newSelected);
  };

  const stableNav = useCallback((path: string) => {
    import('react').then(({ startTransition }) => {
      startTransition(() => navigate(path));
    });
  }, [navigate]);
  const stableEditSys = useCallback((s: System) => onEditSystem(s), [onEditSystem]);
  const stableDelSys = useCallback((id: string) => setDeleteSystemId(id), []);
    ...
    <div>
        ...
        {useVirtualScroll ? (
          <div>
            <div className={`grid items-center gap-2 px-4 py-3 text-sm font-medium ${t.tableHeader} ${t.textSecondary}`}
              style={{ gridTemplateColumns: '40px 1.5fr 1fr 1fr 80px 100px 120px' }}>
              <div>选择</div><div>系统名称</div><div>编号</div><div>生产指令号</div><div>阶段</div><div>状态</div><div>操作</div>
            </div>
            <AutoVirtualList items={filteredSystems} height={Math.min(600, window.innerHeight - 350)} estimatedItemHeight={SYSTEM_ROW_HEIGHT} overscan={5}
              getItemKey={(s) => s.id}
              renderItem={(s) => (
                <SystemRow sys={s} isSelected={selectedSystems.has(s.id)} modules={modules} t={t}
                  onSelect={handleSelectSystem} onNavigate={stableNav} onEdit={stableEditSys} onDelete={stableDelSys} canEdit={canEdit}
                  getSystemStatusInfo={getSystemStatusInfo} />
              )}
            />
          </div>
        ) : (
      ...
      {useVirtualScroll && (
        <div className={`flex items-center justify-between px-4 py-2 ${t.card} rounded-lg border ${t.border} mt-3`}>
          <div className={`text-sm ${t.textSecondary}`}>共 {filteredSystems.length} 条系统（虚拟滚动模式）</div>
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (deleteSystemId) {
            onDeleteSystem(deleteSystemId);
          }
          setDeleteSystemId(null);
        }}
        onCancel={() => setDeleteSystemId(null)}
        type="danger"
        confirmText="确认删除"
        cancelText="取消"
      />
