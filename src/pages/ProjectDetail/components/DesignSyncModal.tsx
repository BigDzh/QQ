...
interface DesignSyncModalProps {
  ...
  onClose: () => void;
  targetFile: any;
  syncModuleIds: string[];
  onSyncModuleIdsChange: (ids: string[]) => void;
  onSync: () => void;
  modules: Module[];
}

export function DesignSyncModal({ show, onClose, targetFile, syncModuleIds, onSyncModuleIdsChange, onSync, modules }: DesignSyncModalProps) {
  const t = useThemeStyles();

  if (!show || !targetFile) return null;

  const adaptedModules = modules.filter((m: Module) => targetFile?.adaptedModuleIds?.includes(m.id));
  const allComponents = modules.flatMap(m => m.components || []);
  const adaptedComponents = allComponents.filter((c: Component) => targetFile?.adaptedComponentIds?.includes(c.id));

  const handleToggle = (id: string) => {
    if (syncModuleIds.includes(id)) {
      onSyncModuleIdsChange(syncModuleIds.filter(i => i !== id));
    } else {
      onSyncModuleIdsChange([...syncModuleIds, id]);
    }
  };

  const allTargets = [...adaptedModules, ...adaptedComponents];
  const isAllSelected = allTargets.length > 0 && syncModuleIds.length === allTargets.length;
  const isIndeterminate = syncModuleIds.length > 0 && syncModuleIds.length < allTargets.length;

  const handleSelectAll = () => {
    if (isAllSelected) {
      onSyncModuleIdsChange([]);
    } else {
      onSyncModuleIdsChange(allTargets.map((t: { id: string }) => t.id));
    }
  };
      ...
      <div className={`${t.modalBg} rounded-lg p-6 w-full max-w-md border ${t.modalBorder} max-h-[90vh] overflow-y-auto shadow-xl`} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`flex items-center gap-2 text-sm ${t.textSecondary}`}>
            <span>文件:</span>
            <span className={`font-semibold ${t.text}`}>{targetFile.name}</span>
          </div>
        </div>
        ...
        <div className={`p-3 rounded-lg mb-4 border ${t.border} ${t.card}`}>
          <div className="flex items-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-emerald-600">{syncModuleIds.length} 个</span>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className={`text-sm ${t.textMuted}`}>
                已选择 <span className={`font-semibold ${t.accentText}`}>{syncModuleIds.length}</span> / {allTargets.length} 个目标
              </div>
            </div>
            ...
            <div className={`border rounded-lg ${t.border} max-h-60 overflow-y-auto`}>
              {adaptedModules.length > 0 && (
                <div>
                  ...
                  <div className="p-2">
                    {adaptedModules.map((module: Module) => {
                      const isSelected = syncModuleIds.includes(module.id);
                      return (
                        <label
                          key={module.id}
              ...
              {adaptedComponents.length > 0 && (
                <div>
                  ...
                  <div className="p-2">
                    {adaptedComponents.map((comp: Component) => {
                      const isSelected = syncModuleIds.includes(comp.id);
                      return (
                        <label
                          key={comp.id}
        ...
        <div className="flex gap-3 pt-4 mt-4 border-t">
          ...
          <button
            onClick={onSync}
            disabled={syncModuleIds.length === 0}
            className={`flex-1 py-2.5 rounded-lg font-medium transition-all ${
              syncModuleIds.length === 0
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : `${t.button} cursor-pointer`
            }`}
          >
            同步 ({syncModuleIds.length})
          </button>
        </div>
      </div>
    </div>
