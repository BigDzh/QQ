...
import type { Software, Module } from '../../../types';

interface SyncModalProps {
  show: boolean;
  onClose: () => void;
  software: Software | null;
  syncComponentIds: string[];
  onSyncComponentIdsChange: (ids: string[]) => void;
  onSync: () => void;
  modules: Module[];
}

export function SyncModal({ show, onClose, software, syncComponentIds, onSyncComponentIdsChange, onSync, modules }: SyncModalProps) {
  const t = useThemeStyles();

  if (!show || !software) return null;

  const allComponents = modules.flatMap(m => m.components || []);
  const adaptedComponents = allComponents.filter(comp =>
    software.adaptedComponentIds?.includes(comp.id)
  );

  const handleToggle = (compId: string) => {
    if (syncComponentIds.includes(compId)) {
      onSyncComponentIdsChange(syncComponentIds.filter(id => id !== compId));
    } else {
      onSyncComponentIdsChange([...syncComponentIds, compId]);
    }
  };

  const isAllSelected = adaptedComponents.length > 0 && syncComponentIds.length === adaptedComponents.length;
  const isIndeterminate = syncComponentIds.length > 0 && syncComponentIds.length < adaptedComponents.length;

  const handleSelectAll = () => {
    if (isAllSelected) {
      onSyncComponentIdsChange([]);
    } else {
      onSyncComponentIdsChange(adaptedComponents.map(c => c.id));
    }
  };
        ...
        <div className={`p-3 rounded-lg mb-4 border ${t.border} ${t.card}`}>
          <div className="flex items-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-emerald-600">{syncComponentIds.length} 个</span>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className={`text-sm ${t.textMuted}`}>
                已选择 <span className={`font-semibold ${t.accentText}`}>{syncComponentIds.length}</span> / {adaptedComponents.length} 个组件
              </div>
            </div>

            <div className={`border rounded-lg ${t.border} max-h-80 overflow-y-auto`}>
              {adaptedComponents.map((comp) => {
                const isSelected = syncComponentIds.includes(comp.id);
                return (
                  <label
                    key={comp.id}
