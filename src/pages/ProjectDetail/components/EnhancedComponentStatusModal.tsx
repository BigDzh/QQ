...
export function EnhancedComponentStatusModal({
  ...
  const showExternalButton = selectedStatus && EXTERNAL_SUBMIT_STATUSES.includes(selectedStatus) && getJumpConfig(selectedStatus)?.enabled && getJumpConfig(selectedStatus)?.url;
  const showExternalWarning = selectedStatus && EXTERNAL_SUBMIT_STATUSES.includes(selectedStatus) && (!getJumpConfig(selectedStatus)?.url || !getJumpConfig(selectedStatus)?.enabled);
