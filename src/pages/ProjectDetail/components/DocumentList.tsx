...
import { Plus, FileText, Edit2, Upload, Trash2, FileUp, Search, X, Loader2, Download, LayoutGrid } from 'lucide-react';
import { useThemeStyles } from '../../../hooks/useThemeStyles';
import { useToast } from '../../../components/Toast';
import { generateId } from '../../../utils/auth';
import type { ProjectStage, Document } from '../../../types';
...
const DOC_ROW_HEIGHT = 48;

interface DocumentRowProps {
  doc: Document; t: ReturnType<typeof useThemeStyles>;
  onEdit: (doc: Document) => void; onUpload: (e: React.ChangeEvent<HTMLInputElement>, id: string) => void;
  onDownload: (doc: Document) => void; onDelete: (id: string, name: string) => void;
  canEdit: boolean; hasFile: boolean; formatFileSize: (s?: number) => string;
}

const DocumentRow = memo(function DocumentRow({ doc, t, onEdit, onUpload, onDownload, onDelete, canEdit, hasFile, formatFileSize }: DocumentRowProps) {
  return (
    <div className={`grid items-center gap-2 px-4 border-t ${t.border} ${t.hoverBg}`}
      style={{ gridTemplateColumns: '1.2fr 1.5fr 0.8fr 0.7fr 0.7fr 0.8fr 0.7fr 140px', height: `${DOC_ROW_HEIGHT}px` }}>
      <div className={`${t.text} truncate`}>{doc.documentNumber}</div>
      <div className={`${t.text} truncate`}>{doc.name}</div>
      <div className={`${t.textSecondary} truncate`}>{doc.type}</div>
      <div className={`${t.textSecondary} truncate`}>{doc.stage}</div>
      <div className={`${t.textSecondary} truncate`}>{doc.version || '-'}</div>
      <div className={`${t.textSecondary} truncate`}>{formatFileSize(doc.fileSize)}</div>
      <div><span className={`px-2 py-1 rounded text-xs ${doc.status === '已完成' ? t.success : t.badge}`}>{doc.status}</span></div>
      <div className="flex items-center gap-1">
        <button onClick={() => onEdit(doc)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="编辑"><Edit2 size={16} /></button>
        <label className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted} cursor-pointer`} title="上传">
          <Upload size={16} />
          <input type="file" accept=".doc,.docx,.pdf" className="hidden" onChange={(e) => { if (e.target.files?.[0]) onUpload(e, doc.id); }} />
        </label>
        {hasFile && (<button onClick={() => onDownload(doc)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="下载"><Download size={16} /></button>)}
        {canEdit && (<button onClick={() => onDelete(doc.id, doc.name)} className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`} title="删除"><Trash2 size={16} /></button>)}
      </div>
    </div>
  );
});
...
export function DocumentList({
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; docId: string | null; docName: string }>({
    show: false,
    docId: null,
    docName: '',
  });
  ...
  const handleExcelImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newDocuments: Partial<Document>[] = jsonData.map((row: Record<string, unknown>, index: number) => {
        const name = String(row['文档名称'] || row['name'] || '');
        if (!name || name.trim() === '') {
  ...
  const useVirtualScroll = filteredDocuments.length > VIRTUAL_SCROLL_THRESHOLD;
    ...
    return (
      <div className="space-y-4">
        ...
        <ConfirmModal
          ...
          message={
            <div>
              <p>确定要删除文档 <strong>"{deleteConfirm.docName}"</strong> 吗？</p>
    ...
    <div className="space-y-4">
          ...
          useVirtualScroll ? (
            <div className="border rounded-lg overflow-hidden" style={{ borderColor: 'var(--tw-border-color, #e5e7eb)' }}>
              <div className={`grid items-center gap-2 px-4 py-3 text-sm font-medium ${t.tableHeader} ${t.textSecondary}`}
                style={{ gridTemplateColumns: '1.2fr 1.5fr 0.8fr 0.7fr 0.7fr 0.8fr 0.7fr 140px' }}>
                <div>文档编号</div><div>文档名称</div><div>类型</div>
                <div>阶段</div><div>版本</div><div>大小</div><div>状态</div><div>操作</div>
              </div>
              <AutoVirtualList items={filteredDocuments} height={Math.min(500, window.innerHeight - 350)} estimatedItemHeight={DOC_ROW_HEIGHT} overscan={5}
                getItemKey={(doc) => doc.id}
                renderItem={(doc) => (
                  <DocumentRow doc={doc} t={t} onEdit={onEditDocument} onUpload={handleUpload}
                    onDownload={onDownloadDocument} onDelete={(id, name) => setDeleteConfirm({ show: true, docId: id, docName: name })}
                    canEdit={canEdit} hasFile={!!doc.fileUrl} formatFileSize={formatFileSize} />
                )}
              />
            </div>
          ) : (
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    {canEdit && (
                      <button
                        onClick={() => {
                          setDeleteConfirm({ show: true, docId: doc.id, docName: doc.name });
                        }}
                        className={`p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${t.textMuted}`}
                        title="删除"
                      >
      ...
      {useVirtualScroll && (
        <div className={`flex items-center justify-between px-4 py-2 ${t.card} rounded-lg border ${t.border} mt-3`}>
          <div className={`text-sm ${t.textSecondary}`}>共 {filteredDocuments.length} 条文档（虚拟滚动模式）</div>
