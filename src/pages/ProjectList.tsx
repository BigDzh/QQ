...
const themeConfig = {
  minimal: {
    stageColors: {
      C阶段: { bg: 'bg-blue-50', text: 'text-blue-600' },
      S阶段: { bg: 'bg-yellow-50', text: 'text-yellow-600' },
      D阶段: { bg: 'bg-orange-50', text: 'text-orange-600' },
      P阶段: { bg: 'bg-green-50', text: 'text-green-600' },
    },
    card: 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm',
    input: 'bg-white border-gray-200 focus:border-gray-400',
    text: 'text-gray-900',
  ...
  light: {
    ...
    card: 'bg-white border-gray-100 hover:border-gray-200 hover:shadow-sm',
    input: 'bg-white border-gray-200 focus:border-gray-400',
    text: 'text-gray-900',
    textSecondary: 'text-gray-500',
  ...
  dark: {
    stageColors: {
      C阶段: { bg: 'bg-blue-500/10', text: 'text-blue-400' },
      S阶段: { bg: 'bg-yellow-500/10', text: 'text-yellow-400' },
      D阶段: { bg: 'bg-orange-500/10', text: 'text-orange-400' },
      P阶段: { bg: 'bg-green-500/10', text: 'text-green-400' },
    },
    card: 'bg-gray-800 border-gray-700 hover:border-gray-600 hover:shadow-sm',
    input: 'bg-white/5 border-gray-700 focus:border-gray-500 text-white',
    text: 'text-white',
  ...
  cyberpunk: {
    stageColors: {
      C阶段: { bg: 'bg-cyan-500/10', text: 'text-cyan-400', border: 'border-cyan-500/30' },
      S阶段: { bg: 'bg-yellow-500/10', text: 'text-yellow-400', border: 'border-yellow-500/30' },
      D阶段: { bg: 'bg-orange-500/10', text: 'text-orange-400', border: 'border-orange-500/30' },
      P阶段: { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/30' },
    },
    card: 'bg-[#161b22] border-white/5 hover:border-cyan-500/30 hover:shadow-lg hover:shadow-cyan-500/10',
    input: 'bg-white/5 border-white/10 focus:border-cyan-500/50 focus:bg-white/10 text-white placeholder-gray-500',
    text: 'text-white',
    textSecondary: 'text-gray-500',
...
export default function ProjectList() {
  ...
  const handleEdit = (project: Project) => {
    ...
    setFormData({
      ...
      stage: project.stage as 'C阶段' | 'S阶段' | 'D阶段' | 'P阶段',
  ...
  return (
    <div>
      ...
      {filteredProjects.length === 0 ? (
        <div className={`text-center py-20 ${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-2xl border ${isDark ? 'border-white/5' : 'border-gray-200'}`}>
          <div className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center ${
            isDark ? 'bg-white/5' : 'bg-gray-100'
          }`}>
            <Rocket className={isDark ? 'text-gray-600' : 'text-gray-300'} size={32} />
          </div>
          <p className={`${t.textSecondary} text-sm mb-1`}>暂无项目</p>
          <p className={isDark ? 'text-gray-600' : 'text-gray-400'}>点击上方按钮创建您的第一个项目</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProjects.map((project) => {
            ...
            return (
              <div key={project.id} className={`group rounded-2xl border transition-all duration-300 overflow-hidden ${t.card} ${
                isCyberpunk ? 'relative' : ''
              }`}>
                {isCyberpunk && (
                ...
                <div className="relative p-5">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${stageColors.bg} ${stageColors.text} ${
                      isCyberpunk && (stageColors as any).border ? `border ${(stageColors as any).border}` : ''
                    }`}>
                      {project.stage}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <div className={`text-xs ${t.textSecondary}`}>
                      种类: <span className={isDark ? 'text-gray-400' : ''}>{project.categories.length}</span>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, stage: e.target.value as 'C阶段' | 'S阶段' | 'D阶段' | 'P阶段' })}
                    className={`w-full px-4 py-2.5 rounded-xl text-sm focus:outline-none transition-colors ${
                      isCyberpunk 
                        ? 'bg-white/5 border border-white/10 text-white focus:border-cyan-500/50' 
                        : isDark
