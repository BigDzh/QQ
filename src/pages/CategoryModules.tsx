...
export default function CategoryModules() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; moduleId: string | null }>({
    show: false,
    moduleId: null,
  });
  ...
  const handleDelete = (e: React.MouseEvent, moduleId: string) => {
    e.stopPropagation();
    setDeleteConfirm({ show: true, moduleId });
  };

  const handleDeleteConfirm = () => {
    if (deleteConfirm.moduleId) {
      deleteModule(project.id, deleteConfirm.moduleId);
    }
    setDeleteConfirm({ show: false, moduleId: null });
  };

  const handleEdit = (e: React.MouseEvent, module: Module) => {
    e.stopPropagation();
    import('react').then(({ startTransition }) => {
      startTransition(() => navigate(`/modules/${module.id}/edit`));
    });
  };
  ...
  return (
    ...
    <ConfirmModal
      ...
      onCancel={() => setDeleteConfirm({ show: false, moduleId: null })}
      confirmText="确认删除"
      cancelText="取消"
      type="danger"
    />
