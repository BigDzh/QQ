...
const ChartWidget: React.FC<{
  ...
  height?: number;
  index?: number;
}> = ({ title, icon: Icon, color, config, height = 250, index: _index = 0 }) => {
  const [isHovered, setIsHovered] = useState(false);
          ...
          <ResponsiveContainer width="100%" height={height}>
            <PieChart>
              <Pie
                ...
                paddingAngle={2}
              >
                {config.data.map((_entry: Record<string, unknown>, idx: number) => (
                  <Cell
                    key={`cell-${idx}`}
    ...
    const normalComponents = (projects || []).reduce(
      (sum, p) =>
        ...
        ((p.modules && Array.isArray(p.modules)) ? p.modules.reduce(
          (s, m) => s + ((m.components && Array.isArray(m.components)) ? m.components.filter((c: { status: string }) => c.status === '正常').length : 0),
          0
        ) : 0),
      0
    );
