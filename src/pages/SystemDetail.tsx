...
import type { Project, System, Module, Component, ProjectStage } from '../types';
...
interface ModuleTreeNode {
  ...
  data: System | Module | Component | null;
}
...
export default function SystemDetail() {
  ...
  const renderTreeNode = (node: ModuleTreeNode) => {
    ...
    const isModuleUnlinked = node.type === 'module' && node.data && !(node.data as Module).systemId;
    const isComponentUnlinked = node.type === 'component' && node.data && !(node.data as Component).moduleId;
        ...
        <div
          ...
          onClick={() => {
            import('react').then(({ startTransition }) => {
              startTransition(() => navigate(`/modules/${node.id}`));
            });
          }}
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {node.type === 'module' && node.data && (
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs text-gray-400">{(node.data as Module).moduleNumber}</span>
                {node.data.holder && (
                  <>
            ...
            {node.type === 'component' && (
              <div className="flex items-center gap-1 mt-0.5">
                <span className="text-xs text-gray-400">{(node.data as Component)?.componentNumber}</span>
                {node.data?.holder && (
                  <>
                    <span className="text-gray-300">•</span>
                    <span className="text-xs text-gray-400">{node.data.holder}</span>
          ...
          {node.type === 'module' && node.data && (
            <div className="flex items-center gap-2 text-xs">
              <span className="flex items-center gap-1 text-gray-400">
                <Package size={12} /> {getComponentsArray(node.data as Module).length}
              </span>
            </div>
          )}
    ...
    try {
      ...
      updateProject(project.id, {
        systems: project.systems?.map((s: System) =>
          s.id === system.id ? { ...s, ...editForm } as System : s
        ),
      });
  ...
  if (!system || !project) {
    return (
      <div className={`min-h-screen ${t.bg} flex items-center justify-center`}>
        <div className={`${t.card} rounded-xl p-8 border ${t.border} max-w-md w-full mx-4 text-center`}>
          <AlertTriangle size={48} className="mx-auto mb-4 text-amber-500" />
          <h2 className={`text-xl font-bold ${t.text} mb-2`}>系统未找到</h2>
          <p className={`${t.textSecondary} mb-6`}>
            无法找到 ID 为 "{id}" 的系统信息，可能已被删除或链接无效。
          </p>
          <button
            onClick={() => navigate('/projects')}
            className={`px-6 py-2 rounded-lg ${t.button} text-white transition-colors`}
          >
            返回项目管理
          </button>
        </div>
                  ...
                  {filteredModules.map((module: Module) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                      import('react').then(({ startTransition }) => {
                        startTransition(() => navigate(`/modules/${module.id}`));
                      });
                    }}
                      >
                        {!module.systemId && (
