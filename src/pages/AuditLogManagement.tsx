...
import type { StateChangeLog, StateChangePriority } from '../types/audit';
import { useToast } from '../components/Toast';
import { useThemeStyles } from '../hooks/useThemeStyles';
import { ConfirmModal } from './ProjectDetail/components/ConfirmModal';
...
export default function AuditLogManagement() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ show: boolean; type: 'audit' | 'statechange' | null }>({
    show: false,
    type: null,
  });
  ...
  const handleClearAudit = () => {
    setDeleteConfirm({ show: true, type: 'audit' });
  };

  const handleClearAuditConfirm = () => {
    clearAuditLogs();
    setLogs([]);
  ...
  const handleClearStateChange = () => {
    setDeleteConfirm({ show: true, type: 'statechange' });
  };

  const handleClearStateChangeConfirm = () => {
  ...
  return (
    <div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap mb-1">
                                ...
                                <span className={`px-1.5 py-0.5 text-xs rounded ${levelConfig[log.level as AuditLevel].color} bg-opacity-20`}>
                                  {levelConfig[log.level as AuditLevel].label}
                                </span>
      ...
      <ConfirmModal
        ...
        message={
          <div>
            <p>确定要清空所有{deleteConfirm.type === 'audit' ? '审计日志' : '状态变更日志'}吗？</p>
            <p className="text-red-500 mt-2">此操作不可撤销，请谨慎操作。</p>
          </div>
        }
        onConfirm={() => {
          if (deleteConfirm.type === 'audit') {
            handleClearAuditConfirm();
          } else if (deleteConfirm.type === 'statechange') {
            handleClearStateChangeConfirm();
          }
        }}
        onCancel={() => setDeleteConfirm({ show: false, type: null })}
        confirmText="确认删除"
        cancelText="取消"
        type="danger"
      />
