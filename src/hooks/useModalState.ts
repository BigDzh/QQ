...
import type { ProjectStage, ProjectVersion, ModuleStatus, System, Software, Document, DesignFile, Component, Module } from '../types';
...
export function useModalState() {
  ...
  const [systemTemplate, setSystemTemplate] = useState<System | null>(null);
  ...
  const [editingDesignFile, setEditingDesignFile] = useState<DesignFile | null>(null);
  ...
  const [syncTargetSoftware, setSyncTargetSoftware] = useState<Software | null>(null);
  ...
  const [syncTargetDesignFile, setSyncTargetDesignFile] = useState<DesignFile | null>(null);
  const [syncDesignModuleIds, setSyncDesignModuleIds] = useState<string[]>([]);

  const [showVersionModal, setShowVersionModal] = useState(false);
  const [editingVersionSoftware, setEditingVersionSoftware] = useState<Software | null>(null);
  ...
  const [copyingComponent, setCopyingComponent] = useState<Component | null>(null);
  ...
  const [componentTemplate, setComponentTemplate] = useState<Component | null>(null);
  ...
  const [fileConflictInfo, setFileConflictInfo] = useState<{
    reviewId: string;
    category: string;
    existingFile: { id: string; name: string; size?: number; type?: string; dataUrl?: string; category?: string };
    newFile: { id: string; name: string; size: number; type: string; uploadDate?: string; uploadedBy?: string; category?: string; dataUrl?: string };
