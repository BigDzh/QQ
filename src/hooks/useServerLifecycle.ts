...
export function registerTerminationEndpoint(app: { post: (path: string, handler: (req: unknown, res: unknown) => void) => void }, serverProcess: { kill: () => void }) {
  if (app && typeof app.post === 'function') {
    app.post('/__terminate', (_req: unknown, res: unknown) => {
      ...
      try {
        ...
        if (serverProcess && typeof serverProcess.kill === 'function') {
          serverProcess.kill();
          response.status(200).json({ success: true, message: 'Server terminated' });
        } else {
          response.status(200).json({ success: true, message: 'Termination acknowledged' });
        }
      } catch (error) {
        logger.error('[ServerLifecycle] Termination error:', error);
        const errorResponse = res as { status: (code: number) => { json: (data: unknown) => void } };
        errorResponse.status(500).json({ success: false, error: 'Termination failed' });
      }
    });
  }
}
