...
const createTestComponent = (overrides: Record<string, unknown> = {}): any => ({
  id: 'comp-001',
  ...
  borrower: undefined,
  logs: [],
  certificates: { 三防证书: '', 质检报告: '' },
  statusChanges: [],
  ...overrides,
});

const createTestModule = (overrides: Record<string, unknown> = {}): Module => ({
  id: 'mod-001',
  projectId: 'proj-001',
...
describe('borrowCascadeService', () => {
  describe('validateModuleComponentRelation', () => {
    it('应该验证包含组件的模块为有效', () => {
      const module = createTestModule({
        ...
        moduleNumber: 'M-TEST-001',
        components: [
          createTestComponent({ id: 'comp-001', componentNumber: 'C-001' }),
        ],
      });
  ...
  describe('executeBorrowCascade', () => {
    ...
    it('不应该借用包含非"正常"状态组件的模块', () => {
      const module = createTestModule({
        status: '正常',
        components: [
          createTestComponent({ id: 'comp-001', componentNumber: 'C-001', status: '借用中' as const, borrower: '其他用户' }),
          createTestComponent({ id: 'comp-002', componentNumber: 'C-002', componentName: '组件2' }),
        ],
      });
  ...
  describe('createBorrowRecord', () => {
    it('应该为模块创建正确的借用记录', () => {
      const module = createTestModule({ components: [] });
...
describe('状态同步规则测试', () => {
  describe('模块状态计算', () => {
    it('当所有组件状态一致时，模块状态应为该状态', () => {
      const module = createTestModule({
        components: [
          createTestComponent({ id: 'comp-001', componentNumber: 'C-001', componentName: '组件1' }),
          createTestComponent({ id: 'comp-002', componentNumber: 'C-002', componentName: '组件2' }),
        ],
      });
    ...
    it('当组件状态不一致时，模块状态应标记为需要重新计算', () => {
      const module = createTestModule({
        ...
        borrower: '用户A',
        components: [
          createTestComponent({ id: 'comp-001', componentNumber: 'C-001', componentName: '组件1', status: '借用中' as const, borrower: '用户A' }),
          createTestComponent({ id: 'comp-002', componentNumber: 'C-002', componentName: '组件2' }),
        ],
      });
    ...
    it('模块借用应该影响所有组件状态', () => {
      const module = createTestModule({
        components: [
          createTestComponent({ id: 'comp-001', componentNumber: 'C-001', componentName: '组件1' }),
          createTestComponent({ id: 'comp-002', componentNumber: 'C-002', componentName: '组件2' }),
          createTestComponent({ id: 'comp-003', componentNumber: 'C-003', componentName: '组件3' }),
        ],
      });
