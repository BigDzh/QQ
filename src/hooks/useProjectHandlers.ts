...
import type { FileToCompress } from '../utils/compression';
import { useApp } from '../context/AppContext';
import { useToast } from '../components/Toast';
import type { useTransfer } from '../components/TransferProgress';
...
import { createPreUpdateSnapshot, rollbackData, getRollbackSnapshot } from '../services/dataMigration';
...
import type { Project, ProjectStage, ProjectVersion, ModuleStatus, Module, Component, ComponentStatus, DesignFile, Software, Document, System, SystemStatus, Review, ReviewFile } from '../types';
import type { ModalState, SystemForm } from './useModalState';
...
export function useProjectHandlers(
  ...
  const {
    ...
    setShowSystemModal,
    setDiagramResult, setShowDiagramModal,
    setDiagramText, setDiagramType,
  ...
  const handleAutoGenerateDesignDiagrams = useCallback(() => {
    ...
    cards.forEach((card) => {
      ...
      const newDesignFile: Omit<DesignFile, 'id'> = {
        name: card.title,
        type: fileType,
  ...
  const handleCreateSingleDesignDiagrams = useCallback((options: {
    ...
    const findExistingDesignFile = (targetId: string, category: string, fileType: string) => {
      return project.designFiles.find((df: DesignFile) =>
        df.isAutoGenerated &&
    ...
    if (targetType === 'module') {
      const module = project.modules.find((m: Module) => m.id === targetId);
      if (!module) {
      ...
      for (const mod of project.modules) {
        const comp = (mod.components || []).find((c: Component) => c.id === targetId);
        if (comp) {
          targetModule = mod;
          component = comp;
          break;
  ...
  const handleVersionUpdateDesignFile = useCallback((file: DesignFile) => {
    if (!file) return;
  ...
  const handleUploadDesignFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>, designFileId: string) => {
    ...
    try {
      ...
      reader.onload = (event) => {
        ...
        const existingFile = project.designFiles.find((df: DesignFile) => df.id === designFileId);
        if (existingFile) {
  ...
  const handleSyncDesignFileToModules = useCallback(() => {
    ...
    syncDesignModuleIds.forEach(id => {
      const isModule = project.modules.some((m: Module) => m.id === id);
      const allComponents = project.modules.flatMap((m: Module) => m.components || []);
      const isComponent = allComponents.some((c: Component) => c.id === id);
      if (isModule) {
        const module = project.modules.find((m: Module) => m.id === id);
        if (module) {
          const existingDesignFile = (module.designFiles || []).find((df: DesignFile) => df.id === syncTargetDesignFile?.id);
          if (!existingDesignFile) {
            updateProject(project.id, {
              modules: project.modules.map((m: Module) =>
      ...
      if (isComponent) {
        updateProject(project.id, {
          modules: project.modules.map((m: Module) => {
            const compIndex = m.components.findIndex((c: Component) => c.id === id);
            if (compIndex !== -1) {
              const existingDesignFile = m.components[compIndex].designFiles?.find(
                (df: DesignFile) => df.id === syncTargetDesignFile.id
              );
              if (!existingDesignFile) {
  ...
  const handleCreateModule = useCallback((e: React.FormEvent, moduleForm: Partial<Module> & Record<string, unknown>) => {
    e.preventDefault();
    const user = { id: currentUser?.id || null, username: currentUser?.username || '系统' };
    const newModuleState: Omit<Module, 'id' | 'logs' | 'statusChanges'> = { projectId: project.id, ...moduleForm, components: [], version: moduleForm.version || 'A版', stage: (moduleForm.stage as ProjectStage) || 'A阶段', category: moduleForm.category || '', status: (moduleForm.status as ModuleStatus) || '正常', moduleNumber: moduleForm.moduleNumber || '', productionOrderNumber: moduleForm.productionOrderNumber || '', moduleName: moduleForm.moduleName || '' };
  ...
  const handleCreateSystem = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    const user = { id: currentUser?.id || null, username: currentUser?.username || '系统' };
    const { editingSystem, systemForm, systemTemplate } = modalState;
    if (editingSystem) {
      ...
      updateSystem(project.id, editingSystem.id, systemForm as Partial<System>);
      ...
      logSystemConfigChange(
        ...
        '更新系统信息',
        previousSystemState,
        systemForm as unknown as Record<string, unknown>,
        [editingSystem.id]
      );
      ...
      try {
        ...
        if (systemTemplate) {
          const templateModules = project.modules.filter((m: Module) => m.systemId === systemTemplate.id);
          for (const templateModule of templateModules) {
            ...
            const newModule = {
              ...
              components: (templateModule.components || []).map((comp: Component) => ({
  ...
  const handleCreateSoftware = useCallback((e: React.FormEvent) => {
    ...
    const allComponents = project.modules.flatMap((m: Module) => m.components || []);
    const adaptedComponents = allComponents.filter((c: Component) => softwareForm.adaptedComponentIds.includes(c.id));
    const adaptedCategories = [...new Set(adaptedComponents
      .map((c: Component) => {
        const module = project.modules.find((m: Module) => m.components.some((comp: Component) => comp.id === c.id));
        return module?.category;
      })
      .filter(Boolean))];
    if (editingSoftware) {
      updateProject(project.id, {
        software: project.software.map((s: Software) =>
          s.id === editingSoftware.id ? {
            ...
            adaptedComponents: adaptedComponents.map((c: Component) => ({ id: c.id, name: c.componentName })),
            adaptedCategories: adaptedCategories.filter((c): c is string => !!c),
          } : s
        ),
      });
  ...
  const handleDeleteSoftware = useCallback((softwareId: string) => {
    projectState.openConfirmModal('确认删除', '确定要删除该软件吗？', () => {
      updateProject(project.id, { software: project.software.filter((s: Software) => s.id !== softwareId) });
      showToast('软件已删除', 'success');
      projectState.closeConfirmModal();
    });
  }, [project, updateProject, showToast, projectState]);

  const handleDownloadSoftware = useCallback(async (soft: Software) => {
    if (soft.fileUrl) {
  ...
  const handleUploadSoftware = useCallback(async (softwareId: string, file: File) => {
    ...
    try {
      ...
      md5 = await calculateFileMD5Async(file, (progress: { percent: number }) => {
  ...
  const handleOpenSyncModal = useCallback((soft: Software) => {
  ...
  const handleSyncSoftwareToComponents = useCallback((software: Software, componentIds: string[]) => {
    ...
    componentIds.forEach(compId => {
      project.modules.forEach((module: Module) => {
        const comp = module.components.find((c: Component) => c.id === compId);
        if (comp) {
  ...
  const handleUpdateDocument = useCallback(() => {
    ...
    if (editingDoc) {
      updateProject(project.id, {
        documents: project.documents.map((d: Document) => d.id === editingDoc.id ? { ...d, ...docForm } : d),
      });
  ...
  const handleUploadDocument = useCallback((docId: string, file: File) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      const updatedDocuments = project.documents.map((d: Document) =>
        d.id === docId
          ? {
              ...d,
              fileName: file.name,
  ...
  const handleCreateDocumentsFromImport = useCallback((newDocuments: Document[][]) => {
  ...
  const getSearchResults = useCallback(() => {
    ...
    project.modules.forEach((m: Module) => {
  ...
  const handleReviewAction = useCallback((reviewId: string, status: '通过' | '不通过' | '需修改') => {
    ...
    const updatedReviews = project.reviews.map((r: Review) =>
      r.id === reviewId ? {
  ...
  const handleReviewFileUploadWithDrag = useCallback(async (reviewId: string, files: FileList | null, category?: string, applyToAllAction?: 'replace' | 'keep') => {
    ...
    const review = (project.reviews || []).find((r: Review) => r.id === reviewId);
    if (!review) return;
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const existingFile = existingFiles.find((f: ReviewFile) => f.name === file.name);
      ...
      if (existingFile && applyToAllAction === undefined) {
        return new Promise<void>((resolve) => {
          modalState.setFileConflictInfo({
            ...
            existingFile,
            newFile: { name: file.name, size: file.size, type: file.type, id: generateId() },
                ...
                reader.onload = () => {
                  ...
                  const updatedReviews = (project.reviews || []).map((r: Review) =>
                    r.id === reviewId
                      ? {
                          ...r,
                          files: [...(r.files || []).filter((f: ReviewFile) => f.id !== existingFile.id), newFileData],
                        }
                      : r
                  );
                  updateProject(project.id!, { reviews: updatedReviews });
  ...
  const handleDownloadReviewFolder = useCallback(async (reviewId: string) => {
    ...
    try {
      ...
      const blob = await compressFolderToZip(review.files as FileToCompress[], review.title);
  ...
  const handleDownloadReviewCategoryFiles = useCallback(async (reviewId: string, category: string) => {
    const review = (project.reviews || []).find((r: Review) => r.id === reviewId);
    const categoryFiles = review?.files?.filter((f: ReviewFile) => f.category === category) || [];
    if (!categoryFiles.length) { showToast('该文件夹没有文件', 'error'); return; }

    try {
      showToast('正在压缩文件夹...', 'info');
      const blob = await compressFolderToZip(categoryFiles as FileToCompress[], `${review!.title}_${category}`);
  ...
  const handleDeleteReviewFile = useCallback((reviewId: string, fileId: string) => {
    projectState.openConfirmModal('确认删除', '确定要删除这个文件吗？此操作不可撤销。', () => {
      const updatedReviews = (project.reviews || []).map((r: Review) =>
        r.id === reviewId ? { ...r, files: (r.files || []).filter((f: ReviewFile) => f.id !== fileId) } : r
      );
      updateProject(project.id!, { reviews: updatedReviews });
  ...
  const handleDeleteReviewCategory = useCallback((reviewId: string, category: string) => {
    const categoryFiles = (project.reviews || []).find((r: Review) => r.id === reviewId)?.files?.filter((f: ReviewFile) => f.category === category || (f.category as string).startsWith(`${category}/`)) || [];
    const subcategories = (project.reviews || []).find((r: Review) => r.id === reviewId)?.categories?.filter((c: string) => c === category || c.startsWith(`${category}/`)) || [];
    projectState.openConfirmModal('确认删除', `确定要删除 "${category}" 文件夹及其所有子文件夹和文件 (${categoryFiles.length}个文件, ${subcategories.length}个子文件夹) 吗？此操作不可撤销。`, () => {
      const updatedReviews = (project.reviews || []).map((r: Review) =>
        r.id === reviewId ? {
          ...r,
          files: (r.files || []).filter((f: ReviewFile) => f.category !== category && !(f.category as string).startsWith(`${category}/`)),
  ...
  const handleDeleteReview = useCallback((reviewId: string) => {
    projectState.openConfirmModal('确认删除评审', '确定要删除这个评审及其所有文件吗？此操作不可撤销。', () => {
      const updatedReviews = (project.reviews || []).filter((r: Review) => r.id !== reviewId);
      updateProject(project.id!, { reviews: updatedReviews });
  ...
  const handleModuleDelete = useCallback((moduleId: string) => {
    const user = { id: currentUser?.id || null, username: currentUser?.username || '系统' };
    const moduleToDelete = project.modules.find((m: Module) => m.id === moduleId);
  ...
  const handleModuleCopy = useCallback((module: Module, newNumber: string) => {
    const newModuleId = generateId();
    const copiedComponents = module.components.map((c: Component) => ({
      ...c,
      id: generateId(),
  ...
  const handleModuleStatusChange = useCallback(async (
    ...
    const user = { id: currentUser?.id || null, username: currentUser?.username || '系统' };
    const module = project.modules.find((m: Module) => m.id === moduleId);
  ...
  const handleBatchDeleteModule = useCallback(async (moduleIds: string[]) => {
    updateProject(project.id, {
      modules: project.modules.filter((m: Module) => !moduleIds.includes(m.id)),
    });
  }, [project, updateProject]);
  ...
  const handleComponentEdit = useCallback((component: Component) => {
    ...
    modalState.setComponentEditForm({
      componentName: nameArray.join(', '), componentNumber: component.componentNumber,
      ...
      holder: holderArray.join(', '), repairOrderNumber: component.repairOrderNumber || '',
  ...
  const handleBatchUpdateComponentStatus = useCallback(async (moduleIdComponentIds: { moduleId: string; componentId: string }[], newStatus: string) => {
    for (const { moduleId, componentId } of moduleIdComponentIds) {
      const module = project.modules.find((m: Module) => m.id === moduleId);
      if (module) {
        const updatedComponents = module.components.map((c: Component) =>
  ...
  const handleComponentStatusUpdate = useCallback((component: Component, newStatus: string, reason?: string) => {
  ...
  const handleComponentCopy = useCallback((component: Component) => {
  ...
  const handleComponentStatusChange = useCallback((component: Component) => {
    modalState.setEditingComponent(component);
    modalState.setComponentStatusForm({ status: component.status, reason: '' });
    modalState.setShowComponentStatusModal(true);
  }, [modalState]);

  const handleSystemEdit = useCallback((system: System) => {
    modalState.setSystemForm({
      ...
      // TODO: SystemStatus 类型与 SystemForm.status 存在细微差异（可能由于类型定义位置不同）
      status: system.status as SystemForm['status'], stage: system.stage, version: system.version || 'v1.0',
    });
  ...
  const handleSystemDelete = useCallback((systemId: string) => {
    const user = { id: currentUser?.id || null, username: currentUser?.username || '系统' };
    const systemToDelete = project.systems.find((s: System) => s.id === systemId);
    ...
    if (systemToDelete) {
      logSystemConfigChange(
        ...
        '删除系统',
        systemToDelete as unknown as Record<string, unknown>,
        {},
        [systemId]
      );
    }
    ...
    updateProject(project.id, {
      modules: project.modules.map((m: Module) =>
        m.systemId === systemId
          ? { ...m, systemId: '', systemNumber: '', systemName: '' }
          : m
      ),
    });

    showToast('系统已删除', 'success');
  }, [project, deleteSystem, updateProject, showToast, currentUser]);
  ...
  const handleBatchDeleteSystem = useCallback(async (systemIds: string[]) => {
    ...
    updateProject(project.id, {
      modules: project.modules.map((m: Module) =>
        systemIds.includes(m.systemId || '')
          ? { ...m, systemId: '', systemNumber: '', systemName: '' }
          : m
      ),
    });
    showToast(`成功删除 ${systemIds.length} 个系统`, 'success');
  }, [project.id, project.modules, deleteSystem, updateProject, showToast]);

  const handleBatchUpdateSystemStatus = useCallback(async (systemIds: string[], newStatus: string) => {
    const updatedSystems = project.systems.map((s: System) =>
      systemIds.includes(s.id) ? { ...s, status: newStatus as SystemStatus } : s
    );
    await updateProject(project.id, { systems: updatedSystems });
    showToast(`成功更新 ${systemIds.length} 个系统状态为 "${newStatus}"`, 'success');
  }, [project, updateProject, showToast]);

  const handleTaskToggleComplete = useCallback((task: Record<string, unknown> & { taskId?: string }) => {
  ...
  const handleComponentEditSubmit = useCallback((e: React.FormEvent) => {
    ...
    if (oldModuleId !== newModuleId) {
      updateComponent(project.id, oldModuleId, editingComponent.id, {
        ...
        stage: (componentEditForm.stage as ProjectStage) || undefined,
      ...
      project.modules.forEach((m: Module) => {
        const comp = m.components.find((c: Component) => c.id === editingComponent.id);
  ...
  const handleStatusChangeWithReason = useCallback(async (
    ...
    try {
      const targetModule = project.modules.find((m: Module) => m.id === moduleId);
      if (!targetModule) {
        return {
          success: false,
          error: '未找到对应的模块',
      ...
      const targetComponent = targetModule.components?.find((c: Component) => c.id === componentId);
      if (!targetComponent) {
        return {
          success: false,
          error: '未找到对应的组件',
      ...
      console.log('[DEBUG] handleStatusChangeWithReason - 开始记录日志', {
        componentId,
        componentName: targetComponent.componentName,
        previousStatus: targetComponent.status,
        newStatus,
        reason
      });
      ...
      console.log('[DEBUG] handleStatusChangeWithReason - 调用 updateComponent', {
        statusChangeReason: reason
      });
      ...
      await new Promise((resolve, reject) => {
        setTimeout(() => {
          try {
            console.log('[DEBUG] updateComponent - 开始更新组件', {
              projectId: project.id,
              moduleId,
              componentId,
              newStatus,
            ...
            if ((newStatus as string) === '三防中' || (newStatus as string) === '三防完成') {
              const match1 = reason.match(/三防单号\s*[:：]\s*([A-Za-z0-9\-_]+)/);
              const match2 = reason.match(/三防\s*[:：]\s*([A-Za-z0-9\-_]+)/);
              const match3 = reason.match(/(\d+)/);
              const orderNumber = match1?.[1] || match2?.[1] || match3?.[1] || `SF-${Date.now().toString(36).toUpperCase()}`;
              updates.protectionOrderNumber = orderNumber;
            }

            if ((newStatus as string) === '维修中' || (newStatus as string) === '维修完成') {
              const match1 = reason.match(/维修单号\s*[:：]\s*([A-Za-z0-9\-_]+)/);
              const match2 = reason.match(/维修\s*[:：]\s*([A-Za-z0-9\-_]+)/);
              const match3 = reason.match(/(\d+)/);
              const orderNumber = match1?.[1] || match2?.[1] || match3?.[1] || `WX-${Date.now().toString(36).toUpperCase()}`;
              updates.repairOrderNumber = orderNumber;
            }

            updateComponent(project.id, moduleId, componentId, updates);

            console.log('[DEBUG] updateComponent - 更新完成');
            resolve(true);
          } catch (err) {
            console.error('[DEBUG] updateComponent - 更新失败', err);
            reject(err);
          }
        }, 300);
      });
  ...
  const handleComponentStatusSubmit = useCallback(() => {
    ...
    if (!editingComponent) { showToast('请先选择组件', 'error'); return; }
    if (!componentStatusForm.reason.trim()) { showToast('请输入状态变更原因', 'error'); return; }
    const moduleId = editingComponent.moduleId;
    const newStatus = componentStatusForm.status;

    const updates: Record<string, any> = {
      status: newStatus as string,
      statusChangeReason: componentStatusForm.reason,
      logs: [...(editingComponent.logs || []), {
        id: generateId(),
        action: `状态变更：${editingComponent.status} → ${newStatus}`,
        timestamp: new Date().toISOString(),
        userId: currentUser?.id || '',
        username: currentUser?.username || '',
    ...
    if (newStatus === '三防中' || newStatus === '三防完成') {
      const orderNumber = componentStatusForm.reason.match(/三防[单号]?\s*[:：]?\s*(\S+)/)?.[1]
        || componentStatusForm.reason.match(/[A-Za-z0-9]+-\d+/)?.[0]
        || `SF-${Date.now().toString(36).toUpperCase()}`;
      updates.protectionOrderNumber = orderNumber;
    }

    if (newStatus === '维修中' || newStatus === '维修完成') {
      const orderNumber = componentStatusForm.reason.match(/维修[单号]?\s*[:：]?\s*(\S+)/)?.[1]
        || componentStatusForm.reason.match(/[A-Za-z0-9]+-\d+/)?.[0]
        || `WX-${Date.now().toString(36).toUpperCase()}`;
      updates.repairOrderNumber = orderNumber;
    }

    updateComponent(project.id, moduleId, editingComponent.id, updates);
    showToast('组件状态已更新', 'success');
  ...
  const handleComponentCopySubmit = useCallback(() => {
    ...
    if (!copyingComponent) { showToast('请先选择要复制的组件', 'error'); return; }
    const targetModule = project.modules.find((m: Module) => m.id === componentCopyForm.moduleId);
  ...
  const handleDesignFileSubmit = useCallback(() => {
    ...
    if (editingDesignFile) {
      updateProject(project.id, {
        designFiles: project.designFiles.map((df: DesignFile) =>
          df.id === editingDesignFile.id ? { ...df, ...designForm } : df
        ),
      });
      showToast('设计文件已更新', 'success');
    } else {
      addDesignFile(project.id, { ...designForm, isAutoGenerated: false, uploadDate: new Date().toLocaleString() } as Omit<DesignFile, 'id'>);
  ...
  const handleSyncDesignOpen = useCallback((file: DesignFile) => {
  ...
  const handleUpdateDesignFileSync = useCallback((file: DesignFile) => {
    if (!file) return;
    ...
    const updatedModules = project.modules.map((mod: any) => {
      ...
      if (adaptedModuleIds.includes(mod.id)) {
        if (!moduleDesignFiles.find((df: DesignFile) => df.id === file.id)) {
      ...
      const updatedComponents = mod.components?.map((comp: any) => {
        ...
        if (adaptedComponentIds.includes(comp.id)) {
          if (!compDesignFiles.find((df: DesignFile) => df.id === file.id)) {
  ...
  const handleSoftwareFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
    modalState.setSoftwareForm({ ...modalState.softwareForm, [field]: value });
  }, [modalState]);

  const handleDocFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
    modalState.setDocForm({ ...modalState.docForm, [field]: value });
  }, [modalState]);

  const handleDesignFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
    modalState.setDesignForm({ ...modalState.designForm, [field]: value });
  }, [modalState]);

  const handleSystemFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
    modalState.setSystemForm({ ...modalState.systemForm, [field]: value });
  }, [modalState]);

  const handleModuleFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
  ...
  const handleReviewFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
  ...
  const handleOpenEditSoftware = useCallback((soft: Software) => {
  ...
  const handleSaveSoftware = useCallback((software: Software, updates: { name?: string; version?: string; adaptedComponentIds?: string[] }) => {
    ...
    const adaptedComponents = allComponents
      .filter((c: Component) => updates.adaptedComponentIds?.includes(c.id))
      .map((c: Component) => ({ id: c.id, name: c.componentName }));
  ...
  const handleOpenEditDocument = useCallback((doc: Document) => {
    modalState.setEditingDoc(doc);
    modalState.setDocForm({
      documentNumber: doc.documentNumber, name: doc.name, type: doc.type,
      stage: doc.stage, version: (doc).version || 'A',
    });
    modalState.setShowDocModal(true);
  }, [modalState]);

  const handleSelectTemplate = useCallback((sys: System) => {
    modalState.setSystemForm({
  ...
  const handleComponentFormChange = useCallback((field: string, value: string | string[] | boolean | undefined) => {
    modalState.setComponentForm((prev: any) => ({ ...prev, [field]: value }));
  }, [modalState]);

  const handleSelectComponentTemplate = useCallback((comp: Component) => {
    modalState.setComponentForm({
      componentNumber: '',
  ...
  const handleComponentSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (modules.length === 1) {
      ...
      if (!targetModuleId) {
        showToast('模块数据异常', 'error');
        return;
      }
    } else {
      const moduleOptions = modules.map((m: Module) => `${m.moduleName} (${m.moduleNumber})`).join(', ');
      const selectedModuleName = prompt(`请输入要添加到的模块名称:\n${moduleOptions}`);
      if (!selectedModuleName) return;
      const matchedModule = modules.find((m: Module) =>
    ...
    logModuleConfigModify(
      targetModuleId,
      modules.find((m: Module) => m.id === targetModuleId)?.moduleName || '未知模块',
      user,
      `在模块中添加组件: ${form.componentName}`,
      '',
      { componentCount: (modules.find((m: Module) => m.id === targetModuleId)?.components?.length || 0) },
      { componentCount: (modules.find((m: Module) => m.id === targetModuleId)?.components?.length || 0) + 1 }
    );

    addComponent(project.id, targetModuleId, newComponent as Omit<Component, 'id' | 'statusChanges'>);
    ...
    try {
      ...
      if (result) {
        ...
        logSystemConfigChange(
          ...
          { stage: previousStage },
          { stage: stage },
          project.modules.map((m: Module) => m.id)
        );
  ...
  return {
    ...
    handleCreateSingleDesignDiagrams,
