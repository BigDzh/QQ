...
export function useProjectHandlers(
    ...
    cards.forEach((card) => {
      ...
      if (!existingFile) {
        const newDesignFile: any = {
          ...
          category: card.type === 'module-assembly' ? 'module' : 'component',
  ...
  const handleCreateSingleDesignDiagrams = useCallback((options: {
    ...
    if (targetType === 'module') {
      ...
      addDesignFile(project.id, newDesignFile);
      showToast(`已为模块 "${module.moduleName}" 创建装配图`, 'success');
    } else {
      ...
      const newDesignFile: any = {
        name: fileType === '装配图' ? `${component.componentName} 装配图` : `${component.componentName} 配套表`,
        type: fileType,
        format: format,
        category: 'component',
        stage: project.stage,
