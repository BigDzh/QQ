...
class PageNavigatorStore {
  ...
  addEntry(path: string, scrollPosition: number = 0, state?: unknown) {
  ...
  savePageState(path: string, state: PageState) {
    this.pageStates.set(path, { ...state, timestamp: Date.now() } as PageState);
  }

  getPageState(path: string): PageState | null {
    const state = this.pageStates.get(path);
    if (!state) return null;
    const age = Date.now() - (state.timestamp as number);
    if (age > 30 * 60 * 1000) {
...
export function usePageNavigator() {
  ...
  const navigateTo = useCallback((path: string, options?: { state?: unknown }) => {
